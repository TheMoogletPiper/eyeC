...
int ft_str_is_lowercase(char *str){
    ...
    for (i=0;str[i]!='\0';i++){
        ...
        if (str[i] > 90 || str[i] < 65){
            c++;    // Check if the str has any non uppercase characters and one up
        }           // c if true
    }
    if (c > 0) {
...
int main()
{
    ...
    char *pprueba = prueba;
    char garbage[10];   //If string hasn't any value associated to it, it may contain garbage value. Testing with a str with no value assigned
